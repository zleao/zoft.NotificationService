...
using zoft.NotificationService.Core.Async.Subscriptions.OneWay;
using zoft.NotificationService.Core.Async.Subscriptions.TwoWay;
...
#pragma warning restore IDE0044 // Add readonly modifier
           ...
           where TMessage : INotificationMessage
        {
            ...
            {
                throw new ArgumentNullException(nameof(asyncDeliveryAction));
            }

            AsyncSubscription asyncSubscription = subscriptionReferenceType switch
            {
                SubscriptionReferenceTypeEnum.Strong => new OneWayAsyncStrongSubscription<TMessage>(asyncActionRunner, asyncDeliveryAction, context),
                SubscriptionReferenceTypeEnum.Weak => new OneWayAsyncWeakSubscription<TMessage>(asyncActionRunner, asyncDeliveryAction, context),
                _ => throw new ArgumentOutOfRangeException(nameof(subscriptionReferenceType), "Subscription Reference Type unexpected: " + subscriptionReferenceType),
            };
            ...
            lock (_lockObject)
            {
                ...
                {
                    LogProvider.Trace($"Adding oneway subscription (MessageType: '{messageType.Name}' Context: '{asyncSubscription.Context}' SubscriptionId: '{asyncSubscription.Id}')");
                    _oneWaySubscriptions[messageType].Add(asyncSubscription);
                }
                else
                {
                    LogProvider.Trace($"Adding new message type with oneway subscription (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
                                                  ...
                                                  SubscriptionReferenceTypeEnum subscriptionReferenceType)
        {
            ...
            AsyncSubscription asyncSubscription = subscriptionReferenceType switch
            {
                SubscriptionReferenceTypeEnum.Strong => new OneWayAsyncStrongSubscription(asyncActionRunner, asyncDeliveryAction, context),
                SubscriptionReferenceTypeEnum.Weak => new OneWayAsyncWeakSubscription(asyncActionRunner, asyncDeliveryAction, context),
                _ => throw new ArgumentOutOfRangeException(nameof(subscriptionReferenceType), "Subscription Reference Type unexpected: " + subscriptionReferenceType),
            };
            lock (_lockObject)
            {
            ...
            where TResult : INotificationResult
        {
            ...
            AsyncSubscription<TResult> asyncSubscription = subscriptionReferenceType switch
            {
                SubscriptionReferenceTypeEnum.Strong => new TwoWayAsyncStrongSubscription<TMessage, TResult>(asyncActionRunner, asyncDeliveryAction, context),
                SubscriptionReferenceTypeEnum.Weak => new TwoWayAsyncWeakSubscription<TMessage, TResult>(asyncActionRunner, asyncDeliveryAction, context),
                _ => throw new ArgumentOutOfRangeException(nameof(subscriptionReferenceType), "Subscription Reference Type unexpected " + subscriptionReferenceType),
            };
            var messageType = typeof(TMessage);
            ...
            lock (_lockObject)
            {
                if (_twoWaySubscriptions.ContainsKey(messageType))
                {
                    ...
                    {
                        LogProvider.Trace($"Replacing twoway subscription for specified context (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
                        _twoWaySubscriptions[messageType][context] = asyncSubscription;
                    }
                    else
                    {
                        LogProvider.Trace($"Adding twoway subscription for specified context (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
                        _twoWaySubscriptions[messageType].Add(context, asyncSubscription);
                    }
                }
                else
                {
                    LogProvider.Trace($"Adding new message type with twoway subscription for specified context (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
        ...
        public void Unsubscribe(SubscriptionToken subscriptionToken)
        {
            if (subscriptionToken != null)
            {
                lock (_lockObject)
                {
                    if (_oneWaySubscriptions.TryGetValue(subscriptionToken.MessageType, out List<ISubscription> oneWaySubscriptions))
                    {
                        var subscriptionToRemove = oneWaySubscriptions.Find(s => s.Id.Equals(subscriptionToken.Id));
                        if (subscriptionToRemove != null)
                        {
                            LogProvider.Trace($"Removing oneway subscription (MessageType: {subscriptionToken.MessageType} Context: {subscriptionToken.Context} SubscriptionId: {subscriptionToken.Id})");
                            _oneWaySubscriptions[subscriptionToken.MessageType].Remove(subscriptionToRemove);
                        }
                    }

                    if (_twoWaySubscriptions.TryGetValue(subscriptionToken.MessageType, out Dictionary<string, ISubscription> twoWaySubscriptions))
                    {
                        if (twoWaySubscriptions?.ContainsKey(subscriptionToken.Context) == true &&
                            twoWaySubscriptions[subscriptionToken.Context] != null &&
                            twoWaySubscriptions[subscriptionToken.Context].Id == subscriptionToken.Id)
                        {
                            LogProvider.Trace($"Removing twoway subscription (MessageType: {subscriptionToken.MessageType} Context: {subscriptionToken.Context} SubscriptionId: {subscriptionToken.Id})");
        ...
        public void Unsubscribe(Type messageType)
        {
            lock (_lockObject)
            {
                if (_oneWaySubscriptions.TryGetValue(messageType, out List<ISubscription> oneWaySubscriptions))
                {
                    foreach (var subscription in oneWaySubscriptions)
                    {
                        LogProvider.Trace($"Removing oneway subscription (MessageType: {messageType} Context: {subscription.Context} SubscriptionId: {subscription.Id})");
                ...
                if (_twoWaySubscriptions.ContainsKey(messageType))
                {
                    LogProvider.Trace($"Removing all twoway subscriptions for type '{messageType}'");
        ...
        public async Task PublishAsync(INotificationMessage message, string context = AsyncSubscription.DefaultContext)
        {
            ...
            {
                LogProvider.Warn("NotificationMessage publishing not allowed - this normally suggests non-specific generic used in calling code");
                return;
            }
            ...
            {
                LogProvider.Trace($"Nothing registered for one-way messages of type '{messageType.Name}' with context '{context}'");
                return;
            }
            ...
            foreach (var subscription in oneWaySubscriptions)
            {
                if (subscription is not OneWayAsyncSubscription voidAsyncSubscription)
                {
                    LogProvider.Warn($"Found one-way subscription that is not a OneWayAsyncSubscription! ({messageType.Name})");
                }
                else
                {
            ...
            if (!allSucceeded)
            {
                LogProvider.Trace("One or more oneway listeners failed - purge scheduled");
        ...
        public async Task DelayedPublishAsync(INotificationMessage message, string context = AsyncSubscription.DefaultContext, bool tryNormalPublish = false)
        {
            ...
            if (tryNormalPublish)
            {
                ...
                {
                    LogProvider.Warn(string.Format("Failed to normal publish the one-way '{0}' message for context '{1}'. Adding it to delayed publish...", messageType.FullName, context));
                }
            }
        ...
        public async Task PublishPendingNotificationsAsync(object currentPublisher, string context = AsyncSubscription.DefaultContext)
        {
            ...
            {
                LogProvider.Warn("PublishPendingNotificationsFor - Context is null or empty. Assuming default");
            ...
            {
                var msgsCount = _pendingNotifications[context].Count(m => m.Sender != currentPublisher);
                if (msgsCount > 0)
                {
                    LogProvider.Trace($"Found {msgsCount} pending one-way message(s) with context '{context}' available for current publisher ({currentPublisher})");
                ...
                {
                    LogProvider.Trace($"Found {_pendingNotifications[context].Count} one-way pending message(s) with context '{context}' but NONE available for current publisher ({currentPublisher})");
                }
            }
            else
            {
                LogProvider.Trace($"No pending one-way messages found with context {context}");
                return;
            }
        }
            ...
            where TResult : INotificationResult
        {
            ...
            {
                LogProvider.Trace($"Nothing registered for two-way async messages of type '{messageType.Name}' with context '{context}'");
                return default;
            }

            if (twoWaySubscriptions.Count() > 1)
                LogProvider.Trace($"Found {twoWaySubscriptions.Count()} two-way messages for type '{messageType}' and context '{context}'. This should not happen. Going to use the first one...");

            if (twoWaySubscriptions.FirstOrDefault() is not TwoWayAsyncSubscription<TResult> twoWaySubscription)
            {
                LogProvider.Warn($"Found a two-way subscription that is not a TwoWayAsyncSubscription! (MessageType: {messageType.Name} Context: {context})");
                return default;
            }
            ...
            {
                LogProvider.Warn($"Two-way subscription is dead. Going to pruge it (MessageType: '{messageType.Name}' Context: '{context}' SubscriptionId: '{twoWaySubscription.Id}')");
        ...
        private void AddToPendingNotificationList(INotificationMessage message, string context)
        {
            ...
            if (string.IsNullOrEmpty(context))
            {
                LogProvider.Warn($"AddToPendingNotificationList - Context of message type '{messageType.Name}' is null or empty. Assuming default");
                context = AsyncSubscription.DefaultContext;
            }
            ...
            lock (_lockObject)
            {
                ...
                LogProvider.Trace($"AddToPendingAsyncNotificationList - Adding one-way message of type '{messageType.Name}' with context '{context}' to pending async notification list");
        ...
        private IEnumerable<ISubscription> GetSubscriptionsFor(Type messageType, string context, SubscriptionDirectionEnum subscriptionDirection)
        {
            ...
            switch (subscriptionDirection)
            {
                case SubscriptionDirectionEnum.OneWay:
                    lock (_lockObject)
                    {
                        if (_oneWaySubscriptions.TryGetValue(messageType, out List<ISubscription> oneWayTypedSubscriptions))
                        {
                ...
                case SubscriptionDirectionEnum.TwoWay:
                    lock (_lockObject)
                    {
                        if (_twoWaySubscriptions.TryGetValue(messageType, out Dictionary<string, ISubscription> twoWayTypedSubscriptions))
                        {
            ...
            lock (_lockObject)
            {
                switch (subscriptionDirection)
                {
                    case SubscriptionDirectionEnum.OneWay:
                        ...
                        {
                            return oneWayMessageSubscriptions?.Any() == true;
                        }
                        break;
                    ...
                    case SubscriptionDirectionEnum.TwoWay:
                        ...
                        {
                            return twoWayMessageSubscriptions?.Any() == true;
                        }
                        break;
                }
            ...
            lock (_lockObject)
            {
                switch (subscriptionDirection)
                {
                    case SubscriptionDirectionEnum.OneWay:
                        ...
                        if (_oneWaySubscriptions.TryGetValue(messageType, out oneWayMessageSubscriptions))
                        {
                            return oneWayMessageSubscriptions?.Any(m => m.Context == context) == true;
                        }
                        break;
                    ...
                    case SubscriptionDirectionEnum.TwoWay:
                        ...
                        if (_twoWaySubscriptions.TryGetValue(messageType, out twoWayMessageSubscriptions))
                        {
                            return twoWayMessageSubscriptions?.Any(m => m.Key == context) == true;
                        }
                        break;
                }
                ...
                switch (subscriptionDirection)
                {
                    case SubscriptionDirectionEnum.OneWay:
                        lock (_lockObject)
                        {
                            ...
                            {
                                if (_oneWaySubscriptions.TryGetValue(type, out List<ISubscription> oneWaySubscriptionsForType) && oneWaySubscriptionsForType != null)
                                {
                                    foreach (var subscription in oneWaySubscriptionsForType.ToArray())
                                    {
                                        if (!subscription.IsAlive)
                                        {
                                            LogProvider.Trace($"Purging one-way subscription (MessageType: {type} Context: { subscription.Context} SubscriptionId: {subscription.Id})");
                    ...
                    case SubscriptionDirectionEnum.TwoWay:
                        lock (_lockObject)
                        {
                            foreach (var type in messageTypes)
                            {
                                if (_twoWaySubscriptions.TryGetValue(type, out Dictionary<string, ISubscription> twoWaySubscriptionsForType) && twoWaySubscriptionsForType != null)
                                {
                                    foreach (var context in twoWaySubscriptionsForType.Keys.ToArray())
                                    {
                                        if (twoWaySubscriptionsForType[context]?.IsAlive == false)
                                        {
                                            LogProvider.Trace($"Purging two-way subscription (MessageType: {type} Context: {context} SubscriptionId: {twoWaySubscriptionsForType[context].Id})");
